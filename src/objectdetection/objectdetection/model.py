...
class YOLOv3:
    ...
    def __init__(self, img_size = 416, conf=0.5, min_bbox_area=400, min_bbox_side=20, debug=True):
    ...
    def detect(self, img):
        ...
        boxes, confidences, class_ids, class_names = self.post_process(img, outputs)
    ...
    def post_process(self, img, outputs):
        ...
        for output in outputs:
            ...
            if confidence > self.conf:
                ...
                #threshold bbox size
                if w*h > self.min_bbox_area and min(w, h) > self.min_bbox_side:
    ...
    def overlay(self, img, boxes, confidences, class_ids):
        ...
        if self.debug:
            ...
            raise Exception("Debug mode is disabled. Print in of overlay with OpenCV is limited to debug mode. Enable debug mode with 'debug=True' argument.")
    ...
    def show(self, img, wait=1):
        ...
        if self.debug:
            ...
            raise Exception("Debug mode is disabled. Show an image with OpenCV is limited to debug mode. Enable debug mode with 'debug=True' argument.")
...
if __name__ == '__main__':
    ...
    img_path1 = Path(__file__).parent.parent.parent.parent  / 'assets/test_img2.jpg'
    img_path2 = Path(__file__).parent.parent.parent.parent  / 'assets/test_img1.png'

    for img_path in [img_path1, img_path2]:
        img = cv2.imread(str(img_path))  
        img_proc = cv2.cvtColor(img, cv2.COLOR_BGR2RGB) # convert to RGB due to model processing with RGB
        t0 = time.time()
